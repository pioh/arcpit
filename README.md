...
### 📦 Дамп ванильных KV из клиента Dota (под Git)
- `bun run refresh:dota-kv` - обновляет `kv/**/01_dota` из установленной Dota 2 (VPK). Запускайте вручную, когда хотите подтянуть новый патч.
- `bun run generate` - берёт существующий `kv/**/01_dota`, строит `02_custom` (трансформеры), мержит с `03_custom`, генерит финальные `game/scripts/npc/npc_*_custom.txt` и TS-пулы для рандома.
